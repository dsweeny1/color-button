...
test("if background color changes from Medium Violet Red to gray when disabled", () => {
  render(<App />);
  const checkbox = screen.getByRole("checkbox", { name: "Disable Button" });
  const colorButton = screen.getByRole("button", {
    name: "Change to Midnight Blue",
...
test("if background color changes from Midnight Blue to gray when disabled", () => {
  render(<App />);
  const checkbox = screen.getByRole("checkbox", { name: "Disable Button" });
  const colorButton = screen.getByRole("button", {
    name: "Change to Midnight Blue",
...
// they're really high-level which makes them resistant to refactors -- more likely to fail when code is refactored
// this makes bugs/failing tests more difficult to diagnose
